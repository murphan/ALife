		...
		for (auto y = 0; y < environment.getHeight(); ++y) {
			for (auto x = 0; x < environment.getWidth(); ++x) {
				...
				if (food.filled() && !food.broken()) {
					organismGrid.accessUnsafe(x, y) = Space::makeFood(x, y);
				}
			}
		}
	}
	...
	auto serialize(const Controls & controls) -> json  {
		...
		if (controls.activeNode != nullptr) {
			...
			if (found != organisms.end()) {
				messageBody.push_back({ "organism", SerializeOrganism::serialize(*found, true) });
			}
		}
